...
class UfoManager {
  constructor() {
    ...
    this.config = {
      ...
      elon: {
        frameDuration: 300,
        displayDuration: 8000,  // How long Elon stays visible before UFO appears
        fadeOutDuration: 90   // How long Elon's fade out animation takes
      }
    };
  }
  ...
  cleanupElonElements() {
    ...
    // Don't call fadeOutElonElement() here anymore since we're doing it in finishUfoAnimation()
    
    // Remove elements after appropriate delay
  ...
  finishUfoAnimation() {
    ...
    if (this.elements.elon) {
      // First change the transform origin to center for predictable rotation
      this.elements.elon.style.transformOrigin = "center center";
      
      // Change the transition properties - faster opacity (shrinking), slower transform (falling)
      this.elements.elon.style.transition = "opacity 0.8s ease, transform 2.5s cubic-bezier(0.4, 0.1, 0.2, 1)";
      
      // Short timeout to ensure the new properties are applied
      setTimeout(() => {
        this.elements.elon.style.opacity = "0";
        // Use negative rotation to tumble forward (downward direction)
        this.elements.elon.style.transform = "scale(0) translate(0px, 300px) rotate(-360deg)";
      }, 10);
      
      // Only remove Elon after the fadeout completes - a bit longer for the slower fall
      setTimeout(() => {
        this.cleanupElonElements();
  ...
  createDebugControls() {
    ...
    container.appendChild(this.createButton("Test Elon Only", () => this.showElonMusk()));

    // Add cleanup button
    container.appendChild(this.createButton("Clean Up Elements", () => this.cleanupElonElements()));
