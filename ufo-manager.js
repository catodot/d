...
class UfoManager {
  ...
  _isGameOver() {
    ...
    return document.body.classList.contains('game-over') || 
           (window.gameEngine && window.gameEngine.gameOver);
  }
  ...
  startUfoAnimation() {
    if (window.audioManager) {
      window.audioManager.play("ui", "aliens");
    }
  ...
  cleanupElonElements(options = {}) {
    ...
    // Apply cartoony, bouncy tumble animation if requested
    if (options.withTumble && this.elements.elon) {
      // Use more cartoony timing function and longer duration
      this.elements.elon.style.transition = `
        transform 1.2s cubic-bezier(0.17, 0.67, 0.4, 1.8), 
        opacity 0.5s ease-in
      `;
      
      // Initial "squash" effect
      setTimeout(() => {
        this.elements.elon.style.transform = `scale(1.3, 0.7) translateY(20px)`;
      }, 10);
      
      // Big bounce with spin
      setTimeout(() => {
        this.elements.elon.style.transform = `
          scale(1.2) 
      ...
      // Add final "flying away" motion with more rotation
      setTimeout(() => {
        this.elements.elon.style.transform = `
          scale(0.2) 
    ...
    // Remove elements after delay
    const removeDelay = options.withTumble ? 1800 : 0;
  ...
  createElonHitbox() {
    ...
    this.elonHitbox.addEventListener("click", (e) => {
      e.stopPropagation();
      if (window.audioManager) {
        window.audioManager.playRandom("defense", "slap");
      }
  ...
  createDebugControls() {
    ...
    const toggleButton = this.createButton(this.state.autoSpawnEnabled ? "Disable Auto UFO" : "Enable Auto UFO", () => {
    ...
    container.appendChild(
      this.createButton("Test Elon Only", () => {
        ...
        setTimeout(() => {
          ...
          if (!document.getElementById("tumble-elon-button")) {
            const tumbleButton = this.createButton("Tumble Elon Away Now", () => {
              this.cleanupElonElements({ withTumble: true }); // Use options object for clarity
            });
    ...
    // Add cleanup button
    container.appendChild(this.createButton("Clean Up Elements", () => this.cleanupElonElements({ withTumble: false })));
